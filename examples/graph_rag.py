...
def extract_graph_components(raw_data):
    ...
    for entry in parsed_response:
        ...
        if target_node and relationship:
            relationships.append({"source": nodes[node], "target": nodes[target_node], "type": relationship})
...
def fetch_related_graph(neo4j_client, entity_ids):
    ...
    with neo4j_client.session() as session:
        ...
        for record in result:
            subgraph.append({"entity": record["e"], "relationship": record["r"], "related_node": record["related"]})
            if record["r2"] and record["n2"]:
                subgraph.append(
                    {"entity": record["related"], "relationship": record["r2"], "related_node": record["n2"]}
                )
    return subgraph
