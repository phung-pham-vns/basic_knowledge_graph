...
def get_cypher_generation_chain(llm: BaseLanguageModel):
    cypher_generation_chain = cypher_generation_prompt | llm | StrOutputParser()
...
text_to_cypher = get_cypher_generation_chain(llm_client)
...
if __name__ == "__main__":
    ...
    examples = [
        ...
        "If my tree has Yellowish-Brown, what disease could it be?",
    ...
    for query in examples:
        ...
        text_to_cypher_response = text_to_cypher.invoke({"schema": graph_client.schema, "question": query})
        print(f"Text to Cypher: {text_to_cypher_response}")
